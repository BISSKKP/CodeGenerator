...
public class CreateController {
	...
	// 创建controller
		public static void createController(String shortName, List<GeneratedJavaFile> mapperJavaFiles,
			...
			controller
					.addImportedType(CommonJavaType.Autowired);
			...
			GeneratedJavaFile fcontroller = new GeneratedJavaFile(controller, PropertiesUtils.get("targetProject"), javaFormatter);
		...
		public static void createDatamethod(String shortName, String shiro, String swagger, TopLevelClass controller) {
			...
			if ("0".equals(swagger)) {
				...
				viewMethod.addAnnotation("@ApiImplicitParams({"
						+ "@ApiImplicitParam(name = \"pageNum\", value = \"pageNum\", dataType = \"int\", required = false, defaultValue = \"1\", paramType = \"query\"),"+StringUtils.getNewLine()
						+ "@ApiImplicitParam(name = \"pageSize\", value = \"pageSize\", dataType = \"int\", required = false, defaultValue = \"10\", paramType = \"query\"),"+StringUtils.getNewLine()
						+ "})");
			}
			...
			viewMethod.setVisibility(JavaVisibility.PUBLIC);
			viewMethod.addBodyLine("return " + StringUtils.tolow(shortName) + "Service.data(" + StringUtils.tolow(shortName) + ",request);");
			controller.addMethod(viewMethod);
		}
		 ...
		 * @param controller
		 */
		public static void createSaveOrUpdatemethod(String shortName, String shiro, String swagger, TopLevelClass controller) {
			...
			viewMethod.setVisibility(JavaVisibility.PUBLIC);
			viewMethod.addBodyLine("return " + StringUtils.tolow(shortName) + "Service.saveOrUpdate(" + StringUtils.tolow(shortName) + ");");
			controller.addMethod(viewMethod);
		}
}
